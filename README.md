...
- 一键覆盖更新板端 app/model
- **独立窗口播放**：用 `ffplay` 在新窗口播放，重复点击“开始播放”不会叠加窗口
- **按钮命令 + 日志控制台**：主窗口底部固定显示每个按钮实际执行的命令行（`$ …`）和运行日志
- **录制 MP4**：播放中可“开始录制 / 停止录制”，录制保存到程序旁边的 `recordings/` 文件夹（`YYYYmmdd_HHMMSS.mp4`），同一播放会话可反复录制多次；“打开录制文件夹”按钮可直接打开该目录
...
播放画面用 `ffplay` 渲染，在**独立的 ffplay 窗口**中显示（不会弹黑色控制台）。主窗口底部是一个固定的**控制台**（“按钮命令 + 日志”），显示每个按钮实际执行的命令行（以 `$` 开头）和运行日志，比如：

```text
[18:30:01] $ adb devices
[18:30:05] $ cd /tmp && /usr/bin/sample_smart_camera --rtsp-only >/tmp/sample_smart_camera.log 2>&1
[18:30:08] $ ffplay -rtsp_transport tcp rtsp://192.168.1.10:8554/ch0
```

- 快速重复点击“开始播放”不会堆积多个视频窗口：同一 RTSP 地址只会启动一个 `ffplay`，已启动时直接复用。
- 点“停止播放”或关闭主窗口会终止 ffplay；视频窗口由 `ffplay` 独立管理。

## 录制 MP4

播放开始后，“开始录制 / 停止录制”按钮可用。点“开始录制”会原样封装当前 RTSP 流（`-c copy`，不重编码）到**程序旁边的 `recordings/` 文件夹**：

```text
<app文件夹>/recordings/YYYYmmdd_HHMMSS.mp4
```

- “停止录制”结束当前录制并生成 mp4 文件；播放停止或退出程序时如果还在录制，会先自动结束并保存。
- 同一播放会话中可以反复“开始录制 → 停止录制”，每次一个独立文件。
- 操作区有个 **“打开录制文件夹”** 按钮，点击直接用系统文件管理器打开 `recordings/` 目录，方便找到录制的视频。
